...
def get_df_list_1st(file_path):
    ...
    for index, page in enumerate(pages):
        # header에 해당되는 테이블 추출
        df_header = get_table_header(index, page.crop((0,0,841,115)))
        
        # content에 해당되는 테이블 추출
        df_content = get_table_content(index, page.crop((0,115,841,595)))
...
def get_df_list_sn(file_path):
    ...
    width = pages[0].bbox[2]
    height = pages[0].bbox[3]
    # header에 해당되는 테이블 추출

    header = pages[0].crop((width*0.05, 0, width*0.95, height*0.15)).extract_text()
    header_list = header.split()

    school_name = [i for i in header_list if '고등학교' in i]
    school_name = school_name.pop()
    class_num = [i for i in header_list if '반' in i]
    class_num = class_num.pop()

    file_name = f"{header_list[0]} {header_list[1]} - {school_name} - {class_num}"

    header_list_result = [file_name, school_name, class_num[:-1]]
    df_list = [header_list_result]
    # PDF 파일의 페이지 반복
    # 한 페이지에서 header와 content 구역을 따로 설정하여 두 개의 테이블을 추출
    for index, page in enumerate(pages):
        # content에 해당되는 테이블 추출
